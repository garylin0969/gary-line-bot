...
class RouteHandler {
	// 處理預載端點
	static async handlePreload(env: Env): Promise<Response> {
	...
	// 處理文案預載端點
	static async handleCopywritingPreload(env: Env): Promise<Response> {
	...
	// 處理LINE事件
	static async handleLineEvents(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		try {
...
class ScheduledTaskHandler {
	// 處理排程事件
	static async handleScheduledEvent(event: ScheduledEvent, env: Env): Promise<void> {
		...
		if (utc8Hour === 0 && utc8Minute === 30) {
			await this.handleHoroscopePreload(env);
		}

		// 每兩小時執行文案預載（在偶數小時的10分鐘執行）
		if (utc8Hour % 2 === 0 && utc8Minute === 10) {
			await this.handleCopywritingPreload(env);
		}
	...
	// 處理運勢預載
	private static async handleHoroscopePreload(env: Env): Promise<void> {
	...
	// 處理文案預載
	private static async handleCopywritingPreload(env: Env): Promise<void> {
...
export default {
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		if (url.pathname === '/preload' && request.method === 'GET') {
			return RouteHandler.handlePreload(env);
		}

		// 手動預載文案端點
		if (url.pathname === '/preload-copywriting' && request.method === 'GET') {
			return RouteHandler.handleCopywritingPreload(env);
		}
		...
		// 處理LINE事件
		return RouteHandler.handleLineEvents(request, env, ctx);
	},

	async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
		await ScheduledTaskHandler.handleScheduledEvent(event, env);
	},
};
