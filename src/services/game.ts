...
// 獲取遊戲狀態
async function getGameState(groupId: string, env: Env): Promise<GameState | null> {
...
// 驗證玩家數量
function validatePlayerCount(input: string): number | null {
	// 檢查是否為純整數（只包含數字）
	if (!/^\d+$/.test(input)) {
...
// 建立新遊戲
async function createNewGame(groupId: string, maxPlayers: number, replyToken: string, accessToken: string, env: Env): Promise<void> {
	const id = env.GAME_STATE.idFromName(groupId);
	const obj = env.GAME_STATE.get(id);
...
// 執行骰子
async function performRoll(groupId: string, userId: string, env: Env): Promise<{ success: boolean; data?: RollResponse; error?: string }> {
	const id = env.GAME_STATE.idFromName(groupId);
	const obj = env.GAME_STATE.get(id);
...
// 處理骰子錯誤
async function handleRollError(error: string, replyToken: string, accessToken: string): Promise<void> {
...
// 處理骰子結果
async function handleRollResult(
	result: RollResponse,
	groupId: string,
	replyToken: string,
	accessToken: string,
